...
(function() {
    ...
    var langHiddenObserver = new MutationObserver(updateShownLang);
    ...
    colorize();
    function colorize() {
        globalLogo = document.querySelector('#globalLogo, .sc-global-logo, .global-logo:not([style]), .logo-wrap img');
        if (!globalLogo) {
            // we're inside some non-Ribbon iframe we don't care about, exit
            return;
        }
        exmObserver.disconnect();
        var logoContainer = globalLogo.parentElement;
        var col = logoContainer.parentElement;
        ...
        if (contentEditor || formsEditor) {
            ...
            envName = `${envName} (<span id="showLang">${curLang}</span>)`;
            let rightCol = col.nextElementSibling;
        ...
        // prep closing x, content-editor pencil and forms-builder form links
        ...
        if (!loginScreen) {
            ...
            if (dbName != '' && !exm) {
                // add buttons to switch database
                var switchTo1 = dbName == 'master' ? 'core' : 'master';
                var continueQuery = '';
                if (!desktop) {
                    continueQuery = location.href.split(location.host)[1]
                        .replace(/[?&]expandTo=[^?&]*/, '')
                        .replace(/[?&]scrollTo=[^?&]*/, '')
                        .replace(/[?&]clickTo=[^?&]*/, '')
                        .replace(/[?&]langTo=[^?&]*/, '')
                ...
                var dbSwitch1 = document.createElement('a');
                ...
                logoContainer.appendChild(dbSwitch1);
                var switchTo2 = dbName == 'web' ? 'core' : 'web';
                var dbSwitch2 = document.createElement('a');
                ...
                logoContainer.appendChild(dbSwitch2);
            }
            if (ribbon) {
                ...
                logoContainer.appendChild(qaContainer);
                renderQuickAccess();
            }
            if (contentEditor) {
